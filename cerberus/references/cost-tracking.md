...
`usage.jsonl` is machine-readable on purpose: the rollup reads structured records and computes exact totals, instead of a human summing markdown tables by eye. Human-readable weekly/monthly reports are *generated on demand* by `report.py` (optionally as markdown) rather than maintained by hand. `CERBERUS.md` is likewise a **derived read-model** — regenerated from `usage.jsonl` after each dispatch, written to the project root, git-ignored, never hand-edited, and still not a per-task ticket. See "Status and the task log" below.
...
- `--summary "…"` records a one-line "what landed" for the task log and `CERBERUS.md`; `--commit HASH` records the commit the dispatch produced. Set these on the implement (and, if it refines the summary, review) dispatch — they're what make the task log readable.
- `--note` adds free text (e.g. why a fallback tier was used).
- `--no-status` skips regenerating `CERBERUS.md` (it's regenerated by default after every dispatch); `--status-file PATH` writes it somewhere other than the default `CERBERUS.md` at the project root.
...
```json
{"date":"2026-07-08","week":"2026-W28","task":"Avatar → profile nav","phase":"implement",
 "status":"ok","model":"claude-sonnet-4-6","input":120,"output":18530,"cache_read":2904110,
 "cache_write":98400,"requests":3,"cost":1.12,"pricing_date":"2026-07-08",
 "summary":"tap avatar → open profile sheet","commit":"a1b2c3d","note":""}
```

`cost` is `null` when the row couldn't be priced honestly (no pricing cache, or the model isn't in it); `pricing_date` records which day's prices were used, so a stale-priced row is auditable. `summary` and `commit` are optional and drive the task log; `status` (`ok`/`bounce`/`bail`) is what the task-status view reads to infer each task's outcome.
...
## Status and the task log

The worklog records one row per *dispatch* (phase-level), but the user usually wants a *task-level* view: what's in flight now, and how recent tasks turned out. That view is **derived** from `usage.jsonl` — grouping a task's rows and reading the latest one — so it needs no separate tracker and no per-task files.

Outcome is inferred from the phase and `status` of a task's most recent row:

- latest row is a **review** that isn't a bounce → ✅ **approved** (done)
- latest row is a review **bounce**, or an **implement** with no review after it → ↻ **in progress** (this is what "in flight" reports)
- latest row is a **bail** → ⚠️ **bailed**
- only a **plan** so far → 📋 **planned**

Two ways to read it:

```bash
python3 cerberus/scripts/report.py --status   # one line: what task is in flight right now
python3 cerberus/scripts/report.py --tasks    # readable log of recent tasks + outcomes (--markdown too)
```

And `CERBERUS.md` (at the project root) is the same dashboard as a file, **regenerated automatically after every `record_usage.py` call** (pass `--no-status` to skip; `report.py --write-status` regenerates it on demand). It shows the in-flight task and a recent-tasks table with outcome, dispatch count (with bounce/bail flags), cost, the `summary` line, and the commit. It's git-ignored and transient, so it never enters source control — a local read-out, not a shared ticket. The task log reads best when implement/review dispatches pass `--summary` (and `--commit`); without them the outcome and cost still show, just no "what landed" line.

Keep `CERBERUS.md` (the transient status mirror) distinct from a **planning doc** like `PRD.md`: the planning doc is the user's own, human-authored backlog of phases and tasks that Cerberus reads from and may tick off on approval; `CERBERUS.md` is generated. See "Working from a planning doc" in `SKILL.md`.
